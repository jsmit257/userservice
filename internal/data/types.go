...
type (
	User interface {
		...
		BasicAuth(ctx context.Context, login *sharedv1.BasicAuth, cid sharedv1.CID) (*sharedv1.User, error)
		// GetUser fetches a user by ID, or returns an error
		GetUser(ctx context.Context, id string, cid sharedv1.CID) (*sharedv1.User, error)
		...
		AddUser(ctx context.Context, u *sharedv1.User, cid sharedv1.CID) (string, error)
		// UpdateUser updates anything except password and salt; that's handled elsewhere
		UpdateUser(ctx context.Context, u *sharedv1.User, cid sharedv1.CID) error
		// DeleteUser shouldn't ever be called
		DeleteUser(ctx context.Context, id string, cid sharedv1.CID) error
		// CreateContact adds a contact to the current user
		CreateContact(ctx context.Context, id string, c *sharedv1.Contact, cid sharedv1.CID) (string, error)
	}

	Address interface {
		// GetAddress fetches an Address by ID
		GetAddress(ctx context.Context, id string, cid sharedv1.CID) (*sharedv1.Address, error)
		// AddAddress inserts a row into Addresses and returns the primary key, or an error
		AddAddress(ctx context.Context, addr *sharedv1.Address, cid sharedv1.CID) (string, error)
		...
		UpdateAddress(ctx context.Context, addr *sharedv1.Address, cid sharedv1.CID) error
		// DeleteAddress shouldn't ever be called
		DeleteAddress(ctx context.Context, id string, cid sharedv1.CID) error
	}

	Contact interface {
		// GetContact returns Contact details with non-nill user object and optional billto
		// and shipto properties
		GetContact(ctx context.Context, userID string, cid sharedv1.CID) (*sharedv1.Contact, error)
		...
		// the API
		AddContact(ctx context.Context, c *sharedv1.Contact, cid sharedv1.CID) (string, error)
		...
		UpdateContact(ctx context.Context, c *sharedv1.Contact, cid sharedv1.CID) error
		// DeleteContact shouldn't ever be called
		DeleteContact(ctx context.Context, id string, cid sharedv1.CID) error
		// SetDefaultBillto
		// SetDefaultShipto
		// GetBillTos
		// GetShipTos
